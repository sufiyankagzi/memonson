...
const SizeGroup = () => {
  ...
  const [sizegroup, setSizeGroups] = useState([]);

  const fetchSizeGroups = async () => {
    try {
      const res = await axios.get("http://localhost:5000/sizegroup/getsizegroup");
      setSizeGroups(res.data);
    } catch (err) {
      console.error(err);
      dispatch(showToast({ message: "Failed to fetch size group", type: "error" }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!sizegroupname.trim()) {
      dispatch(showToast({ message: "Size group name is required", type: "error" }));
      return;
    }
    try {
      const res = await axios.post("http://localhost:5000/sizegroup/newsizegroup", {
        sizegroupid: 1,
        sizegroupname: sizegroupname,
      });
      if (res.data.data.insertId) {
        dispatch(showToast({ message: "Size Group Successfully Created", type: "success" }));
        setSizeGroupName("");
        
        fetchSizeGroups(); // refresh table
      }
    } catch (err) {
      console.error(err);
  ...
  // 🔥 Delete group
  const deleteSizeGroup = async (id) => {
    try {
      const res = await axios.delete(`http://localhost:5000/sizegroup/deletesizegroup/${id}`);
      dispatch(showToast({ message: res.data.message, type: "error" }));
      fetchSizeGroups();
    } catch (err) {
      console.error(err);
      dispatch(showToast({ message: "Failed to delete size group", type: "error" }));
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold font-roboto">Size Group</h2>
        <button
                    ...
                    <div className="bg-white p-5 rounded-3xl shadow w-96">
                      <div className="flex items-center justify-between px-2 pb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <h3 className="text-lg font-bold font-roboto">Size Group</h3>
                        </div>
                        <MdOutlineClose
